    ...
    items.forEach((item) => {
      ...
      for (let i = 0; i < qty; i++) {
        const atom = {
          ...item,
          qty: 1,
          invoId: item.hasOwnProperty("invoId") ? item.invoId ?? undefined : undefined,
...
*/
  ...
  function areEqual(b: Record<string, any>, m: Record<string, any>): boolean {
    // Canonical equality: same item and same receipt context (including both missing)
    const itemMatch = b.itemId === m.itemId;
    const invoMatch = (b.invoId ?? null) === (m.invoId ?? null);
